...
HRESULT CParticleMesh::Initialize(void* pArg)
{
	...
	case CIRCLE:
		...
		break;
	case FORK:
		m_ModelPrototypeTag = TEXT("Prototype_Component_Model_Fork");
		break;
	case SLASH:
...
{
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_EFFECT, strModelPrototype,
		TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_InstModelCom))))
		return E_FAIL;

	/* For.Com_Shader */
	if (FAILED(__super::Add_Component(LEVEL_EFFECT, TEXT("Prototype_Component_Shader_VtxInstance_Mesh"),
