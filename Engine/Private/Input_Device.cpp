...
_bool CInput_Device::MouseButtonUp(MOUSEKEYSTATE eMouse)
{
	static bool prevLeftButtonDown = false;
	bool currentLeftButtonDown = m_tMouseState.rgbButtons[eMouse] & 0x80 ? true : false;

	// Check if the left mouse button was pressed on the previous update but is now released
	bool leftButtonReleased = prevLeftButtonDown && !currentLeftButtonDown;

	// Update the previous left button state for the next frame
	prevLeftButtonDown = currentLeftButtonDown;
...
_bool CInput_Device::isKeyUP(_int _iKey)
{
...
_bool CInput_Device::isKeyDown(_int _iKey)
{
	if (!m_bKeyState[_iKey] && (GetAsyncKeyState(_iKey) & 0x8000))
	{
		m_bKeyState[_iKey] = !m_bKeyState[_iKey];
