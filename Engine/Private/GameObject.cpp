...
HRESULT CGameObject::Set_WorldMat(_float4x4 worldmat)
{
	m_pTransformCom->Set_WorldMatrix(worldmat);
	return S_OK;
}

vector<string> CGameObject::Get_AnimNameVec()
{
	return static_cast<CModel*>(Get_Component(TEXT("Com_Model")))->Get_AnimNameVec();
}

_bool CGameObject::InterSected(CGameObject* Dst)
{
	CCollider* dstCollider = static_cast<CCollider*>(Dst->Get_Component(TEXT("Com_Collider")));
	return static_cast<CCollider*>(Get_Component(TEXT("Com_Collider")))->Intersect(dstCollider);
}
